...
function BuildOpenVisus {
   ...
   SOURCE_DIR=$(pwd)
	...
	if ((OS_OSX==1)); then
		...
		# embedding 
		LD_LIBRARY_PATH=$(pwd):$(dirname ${PYTHON_LIBRARY}) PYTHONPATH=$(pwd) bin/visus
		echo "Embedding mode is working"
		
		# extending 
		LD_LIBRARY_PATH=$(pwd) PYTHONPATH=$(pwd) ${PYTHON_EXECUTABLE} -c "import OpenVisus"
		echo "Extending mode is working"		
...
function InstallQtForCentos {
	...
	./configure --prefix=$BUILD_DIR -R \\\$$ORIGIN \
		 -D _X_INLINE=inline \
	...
	make install
	popd
	Qt5_DIR=$BUILD_DIR/Qt-${QT_VERSION}/lib/cmake/Qt5/
}
