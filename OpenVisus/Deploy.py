...
class DeployUtils:
	...
	def CreateScript(name,env):
		...
		def addEnv(Key,Value):
			setEnv(Key,Value,bAdd=True)
		
		lines=[]
		if WIN32:
			lines+=[DeployUtils.setEnv("this_dir","%~dp0")]
			lines+=[DeployUtils.setEnv("PYTHON_EXECUTABLE",sys.executable)]
			lines+=[DeployUtils.addEnv("PATH",";".join(["%this_dir%\\bin",os.path.dirname(sys.executable)]))]
			
			for key in env: 
				lines+=[DeployUtils.setEnv(key,env[key])]
			...
			lines+=[DeployUtils.setEnv("PYTHON_EXECUTABLE",sys.executable)]
			lines+=[DeployUtils.addEnv("PYTHONPATH",":".join(["${this_dir}"] + sys.path))]
			lines+=[DeployUtils.addEnv(LD_LIBRARY_PATH,LIBDIR)]
			
			for key in env: 
				lines+=[DeployUtils.setEnv(key,env[key])]
				
			lines+=["${this_dir}/{}{}/$@".format(exe,"/Contents/MacOS/{}".format(name) if APPLE else "") ]
		
		ext="bat" if WIN32 else ("command" if APPLE else "sh")
		script_filename="%s.%s" % (name,ext)
